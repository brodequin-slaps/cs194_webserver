...
void thread_work(int client_sockid, filesystem::path path)
{
    sleep(10); // simulate intense computational work
    int status;
...
void webserver_thread_per_client(std::filesystem::path path)
{
    int status;
    ...
    while (keepRunning == 1)
    {
        ...
        thread t1(thread_work, client_sockid, path);
        t1.detach();
    }
